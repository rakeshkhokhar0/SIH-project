...
function deriveRisk(activity) {
  if (activity.status === 'Completed') return 'Low';
  const pct = activity.targetQty > 0 ? activity.completedQty / activity.targetQty : 0;
...
function Reports() {
  ...
  const varianceData = activities.map((a) => ({
    ...a,
    riskScore: deriveRisk(a),
    variance: (a.targetQty || 0) - (a.completedQty || 0),
  }));
    ...
    <div className="px-5 md:px-10 py-7 md:py-9 max-w-5xl">
      ...
      {activities.length === 0 ? (
        <div className="text-center text-[#14213D]/50 py-16 border border-dashed border-[#14213D]/20 rounded-lg">
          No site data logged yet. Once a Site Engineer submits progress, reports will appear here.
        </div>
      ) : (
        <>
            ...
            <div className="hidden md:block border-t border-[#14213D]/10">
              ...
              {varianceData.map((a) => (
                <div key={a.id} className="grid grid-cols-[90px_1.6fr_120px_120px_120px] gap-6 py-3.5 items-center border-b border-[#14213D]/10">
                  ...
                  <div className="text-sm text-[#14213D]/60">{a.targetQty}</div>
                  <div className="text-sm text-[#14213D]/60">{a.completedQty}</div>
            ...
            <div className="md:hidden border-t border-[#14213D]/10">
              {varianceData.map((a) => (
                <div key={a.id} className="py-3.5 border-b border-[#14213D]/10 space-y-1.5">
                  ...
                  <div className="text-sm text-[#14213D]">{a.activityName}</div>
                  <div className="text-xs text-[#14213D]/50">Planned: {a.targetQty} · Actual: {a.completedQty}</div>
                </div>
              ))}
            </div>
          </div>
